...
$env.config.edit_mode = "emacs" # TODO: Consider this
$env.config.completions.algorithm = "fuzzy"
# $env.config.completions.external.completer = null # TODO: Setup carapace
...
export def --env n [
    ...
    let nnn_tmpfile = $env | default '~/.config/' 'XDG_CONFIG_HOME' | get 'XDG_CONFIG_HOME' | path join 'nnn/.lastd' | path expand
    # Launch nnn. Add desired flags after `^nnn`, ex: `^nnn -eda ...$args`,
    # or make an alias `alias n = n -eda`.
    if $selective { ^nnn ...$args } else { ^nnn ...$args }
    if ($nnn_tmpfile | path exists) {
        ...
        let path = open $nnn_tmpfile | str replace --all --regex `^cd '|'$` `` | str replace --all `'\''` `'`
        ^rm -- $nnn_tmpfile
        cd $path
    }
}
